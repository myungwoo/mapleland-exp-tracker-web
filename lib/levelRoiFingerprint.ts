 ...
 * 지문은 `lib/canvas.ts`의 `preprocessLevelCanvas`가 쓰는 전경 판정(`isLevelGlyphPixel`)과
 ...
 *
 * `preprocessLevelCanvas`와 이 지문이 **같은 규칙을 써야** 위에 적은 "지문이 같으면 결과도 같다"가
 * 성립합니다. 그래서 규칙을 이 한 곳에만 두고 양쪽이 함께 import합니다.
