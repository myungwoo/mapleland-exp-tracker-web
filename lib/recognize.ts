import { expValueHasUnknownPrefix, parsePixelExpText, recognizePixelFontLine } from "./pixelRecognizer";
import { recognizeLevelPixelFont } from "./levelPixelRecognizer";

/**
 * 인식 진입점
 *
 * 경험치(EXP)와 레벨(LEVEL) **둘 다** 픽셀 글꼴 템플릿 매칭으로 읽습니다.
 *
 * ⚠️ 범용 인식 엔진(LSTM 기반 OCR 라이브러리)을 다시 들이지 마세요. 한 번 걷어낸 이유입니다:
 * - 두 텍스트 모두 안티에일리어싱이 없는 고정 픽셀 패턴이라, 픽셀 단위로 맞춰보는 쪽이 훨씬 정확합니다.
 * - LSTM 인식기는 애매할 때도 **확신에 찬 틀린 값**을 돌려줍니다. 레벨 193을 183으로 읽는 식인데,
 *   틀린 레벨은 누적 EXP 계산을 통째로 오염시킵니다. 템플릿 매칭은 애매하면 `null`을 돌려주고,
 *   그러면 그 샘플을 건너뛸 뿐입니다.
 * - 런타임에 CDN에서 wasm 코어와 언어 모델을 받아와야 했습니다. (경로에 따라 약 5.8MB ~ 14.4MB)
 *   이 앱은 서버가 없는 정적 배포라 외부 CDN 의존은 그 자체로 부담입니다.
 * - 워커 큐, 주기적 워커 재시작, 2패스 폴백, 문자 혼동 보정(O→0, l/I/|→1) 같은 부속 코드가
 *   전부 딸려옵니다.
 *
