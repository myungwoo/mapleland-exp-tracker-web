 ...
 * **일관되게** 잘못 읽는다면 막지 못합니다. 두 번 읽어도 같은 값이 나오니 확인을 통과하고,
 * 재검증도 같은 값을 다시 확인할 뿐입니다. 다만 그 경우에도 **캐시가 상황을 악화시키지는
 * 않습니다.** 캐시가 없어도 매 샘플 같은 픽셀을 같은 인식기에 넣어 같은 값을 얻기 때문입니다.
 *
 * 즉 일관된 오인식은 캐시 문제가 아니라 인식기 문제입니다. 그래서 레벨 인식을 범용 인식 엔진에서
 * 픽셀 템플릿 매칭(`lib/levelPixelRecognizer.ts`)으로 바꿨습니다. 지금 인식기는 애매하면 값을 찍지 않고
 * `null`을 돌려주므로, "확신에 찬 틀린 레벨" 자체가 나오지 않습니다.
 *
