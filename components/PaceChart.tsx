...
function filterTicksByPixelGap(ticksAsc: number[], scaleY: (v: number) => number, minGapPx: number, eps: number) {
...
export default function PaceChart(props: Props) {
	...
	const chart = useMemo(() => {
		...
		const yTicks = showAxisLabels ? filterTicksByPixelGap(yTicksRaw, yScale, MIN_Y_LABEL_GAP_PX, eps) : yTicksRaw;
