		...
		switch (step) {
			...
			case 1:
				return (
					<>
						<p className="text-white/90">
							화면의 레벨 숫자만 포함되도록 영역을 드래그해서 지정하세요. 영역 안에{" "}
							<span className="font-semibold">&quot;LV.&quot;</span> 텍스트는 포함하지 마세요. 게임 창 크기를 바꾸면
							ROI를 다시 설정해야 합니다.
						</p>
						...
						{props.hasLevelRoi ? (
							<div className="mt-3 p-3 rounded border border-white/15 bg-white/5">
								<div className="text-sm text-white/80 mb-2">
									이미 선택한 ROI가 있습니다. 확인하고 이상이 없으면 건너뛰셔도 됩니다.
								</div>
						...
						<div className="mt-4">
							<button className="btn" onClick={props.onActivateLevel}>
								레벨 ROI 설정
							</button>
						</div>
					</>
			...
			case 2:
				return (
					<>
						<p className="text-white/90">
							경험치 문자열 전체가 들어오도록 선택하세요. <span className="font-semibold">&quot;EXP.&quot;</span>{" "}
							텍스트는 제외하고, 맨 왼쪽 숫자부터 맨 오른쪽 닫는 괄호 <span className="font-mono">]</span>까지 포함되게
							하세요. 위아래로 약간의 여백을 두면 인식이 더 정확합니다. 게임 창 크기를 바꾸면 ROI를 다시 설정해야
							합니다.
						</p>
						<div className="mt-3 p-3 rounded border border-white/15 bg-white/5">
							<div className="text-sm text-white/80 mb-2">올바른 선택 예시</div>
						...
						<div className="mt-4">
							<button className="btn" onClick={props.onActivateExp}>
								경험치 ROI 설정
							</button>
						</div>
					</>
			...
			case 3:
				...
				return (
					<>
						<p className="text-white/90">
							측정으로 인해 렉이 느껴진다면 측정 주기를 늘려 보세요. 기본값은 <span className="font-semibold">1초</span>
							입니다.
						</p>
						<div className="mt-4 flex items-center gap-2">
							{options.map((s) => {
			...
			case 4:
				return (
					<>
						<p className="text-white/90">
							PiP(Document Picture‑in‑Picture) 모드를 사용하면 작은 항상 위 창으로 진행 상황을 볼 수 있습니다. 측정
							시작/일시정지, 초기화를 PiP 창에서도 바로 할 수 있습니다.
						</p>
						<div className="mt-4 flex items-center gap-2">
							<div className="relative inline-block group">
								...
								{props.pipSupported === false && (
									...
									>
										이 브라우저에서는 문서 PiP(Document Picture-in-Picture) 기능을 지원하지 않습니다. 이 기능을
										사용하려면 최신 버전의 Chrome 또는 Edge 브라우저를 이용해 주세요.
									</div>
								)}
							</div>
		...
		>
			...
			>
				<div className="flex items-start gap-3">
					...
					<button className="btn" onClick={props.onClose} aria-label="튜토리얼 닫기" title="닫기">
						닫기
					</button>
				</div>
				<div className="mt-6 flex items-center justify-between">
					<button className="btn text-white/80" onClick={props.onSkip}>
						건너뛰기
					</button>
