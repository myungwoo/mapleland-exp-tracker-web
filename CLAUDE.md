...
⚠️ **기준값(`recognitionSilenceLimitMs`)을 조이지 마세요.** 주기의 3배를 쓰는 이유는 인식이 주기보다 오래 걸려 한두 틱 밀리는 것이 정상이기 때문입니다(§4의 in-flight 가드). 그리고 **탭이 숨겨져 있으면 90초로 늘립니다** — 브라우저가 백그라운드 타이머를 최대 1분 주기까지 늦추는 것은 정상 동작인데, 이 앱은 게임을 전체 화면에 띄우고 쓰므로 숨겨진 상태가 오히려 기본에 가깝습니다. 여기서 오탐이 나면 경고가 늘 떠 있게 되어 알림 전체가 무시됩니다.
