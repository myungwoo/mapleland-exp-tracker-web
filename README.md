...
이런 글꼴은 범용 문자 인식 엔진이 전제하는 "안티에일리어싱된 벡터 글꼴"과 성질이 완전히 달라서, 6/8/3/9를 심하게 혼동합니다.
확대해도 계단 모양 블록이 될 뿐이라 오히려 더 나빠집니다.

그래서 EXP와 레벨 **둘 다** 범용 인식 엔진을 쓰지 않고, **글리프를 픽셀 단위로 대조하는 전용 인식기**로 읽습니다.

|                     |                                                                                                                                           |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| **정확도**          | 픽셀 패턴이 고정이라 맞거나 아예 안 맞거나 둘 중 하나. 조금이라도 확신이 없으면 그 tick을 통째로 버리므로 **틀린 값이 누적되지 않습니다** |
| **디스플레이 배율** | 100% / 125% / 150% / 200% 자동 감지                                                                                                       |
| **속도**            | 무거운 인식 엔진을 돌리지 않아 CPU를 훨씬 적게 씁니다                                                                                     |
...
- 지정한 ROI 영역의 글자를 **브라우저 안에서** 읽습니다.
  ...
  - **레벨**: 오렌지 타일 위 숫자도 같은 방식의 **글리프 템플릿 매칭** (레벨 전용 글꼴, 자체 구현)
  - 외부 인식 엔진이나 런타임에 내려받는 모델이 **없습니다.** 인식 코드는 전부 이 저장소 안에 있습니다.
...
- 글자 인식은 브라우저 내부(EXP·레벨 모두 메인 스레드의 픽셀 글꼴 템플릿 매칭)에서 동작합니다. 외부 CDN에서 인식 모델을 받아오지 않습니다.
...
EXP와 레벨은 글꼴이 서로 달라서 인식기도 따로 있습니다. 진입점은 `lib/recognize.ts` 하나입니다.

|          | 글꼴                 | 템플릿                  | 인식 로직                     | 추출·검증 도구      |
| -------- | -------------------- | ----------------------- | ----------------------------- | ------------------- |
| **EXP**  | 5x7px                | `lib/pixelFont.ts`      | `lib/pixelRecognizer.ts`      | `tools/pixel-font/` |
| **레벨** | 7x7px 볼드, 2배 렌더 | `lib/levelPixelFont.ts` | `lib/levelPixelRecognizer.ts` | `tools/level-font/` |

템플릿은 **반드시 실제 게임 캡처에서** 뽑아야 합니다. (비슷한 시스템 폰트에서 유추하면 틀립니다.
특히 레벨 글꼴을 EXP 글꼴에서 볼드로 유도하면 폭이 1px씩 어긋나 **조용히 오인식**합니다)
패치로 글꼴이 또 바뀌면 아래 도구로 다시 뽑으면 됩니다.

```bash
# 자체 검증 전체 (EXP 인식기 / 레벨 인식기 / 레벨 ROI 변화 감지)
npm test
...
- React 18 + TypeScript
- Tailwind CSS
- 비트맵(픽셀) 글꼴 템플릿 매칭 (EXP·레벨) — 의존성 없는 자체 구현
...
글자 인식기는 외부 라이브러리 없이 이 저장소 안에서 직접 구현했습니다. 번들에 포함된 글꼴은 아래와 같습니다.

- [Pretendard](https://github.com/orioncactus/pretendard) (본문) — SIL Open Font License 1.1, Kil Hyung-jin
- [D2Coding](https://github.com/naver/d2codingfont) (숫자/모노) — SIL Open Font License 1.1, NAVER

두 글꼴 모두 서브셋으로 재배포하고 있으며, 라이선스 전문은 `public/fonts/`에 함께 담겨 있습니다.
