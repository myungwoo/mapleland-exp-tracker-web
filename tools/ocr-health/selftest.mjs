 ...
 *
 *   node tools/ocr-health/selftest.mjs
 *
...
const {
	...
	OCR_STALL_GRACE_MS
} = await loadLibModules(["ocrHealth"], "ocrHealth");
...
function feed(state, args, now) {
	return applyOcrOutcome(state, classifyOcrOutcome(args), now);
}
...
// ---- 원인 분류 ----
check("정상 샘플은 ok", classifyOcrOutcome(OK) === "ok");
check("둘 다 못 읽으면 no_signal", classifyOcrOutcome(BLACK) === "no_signal");
check("경험치만 못 읽으면 exp_missing", classifyOcrOutcome(EXP_GONE) === "exp_missing");
check("레벨만 못 읽으면 level_missing", classifyOcrOutcome(LEVEL_GONE) === "level_missing");
check("이상치 사유가 그대로 원인", classifyOcrOutcome(MISMATCH) === "pct_value_mismatch");
check("급락 사유가 그대로 원인", classifyOcrOutcome(DROP) === "implausible_drop");
check(
	"기록됐다면 사유가 있어도 ok",
	classifyOcrOutcome({ ...OK, outlierReason: "implausible_drop" }) === "ok",
	"기록된 샘플을 실패로 세면 경고가 오작동합니다"
);
check(
	"모르는 이상치 사유는 unknown으로 드러남",
	classifyOcrOutcome({ isRecorded: false, levelRead: true, expRead: true, outlierReason: "새로운_사유" }) === "unknown",
...
// ---- 유예 시간: 포탈 이동처럼 짧은 실패는 경고하지 않습니다 ----
{
	...
	for (let t = 1000; t <= 3000; t += 1000) {
		st = feed(st, BLACK, t);
		if (describeOcrHealth(st, t, { active: true })) warned = true;
	}
	check("3초 검은 화면에는 경고하지 않음", !warned, "포탈 이동마다 경고가 뜨면 사용자가 경고를 무시하게 됩니다");

	// 화면이 돌아오면 즉시 정상입니다.
	st = feed(st, OK, 4000);
	check("복귀하면 경고 없음", describeOcrHealth(st, 4000, { active: true }) === null);
...
// ---- 유예 시간을 넘기면 경고합니다 ----
{
	...
	const justBefore = describeOcrHealth(st, 1000 + OCR_STALL_GRACE_MS - 1, { active: true });
	check("유예 시간 직전에는 경고 없음", justBefore === null);
	const notice = describeOcrHealth(st, 1000 + OCR_STALL_GRACE_MS, { active: true });
	...
	check("지속 시간은 첫 실패 기준", notice?.stalledMs === OCR_STALL_GRACE_MS, `stalledMs=${notice?.stalledMs}`);
}
...
// ---- 지속 시간은 "연속 실패의 시작"부터 셉니다 ----
{
	...
	const notice = describeOcrHealth(st, 30_000, { active: true });
...
// ---- 측정 중이 아니면 경고하지 않습니다 ----
{
	...
	check("측정 중이면 경고", describeOcrHealth(st, 60_000, { active: true }) != null);
	check(
		"측정 중이 아니면 경고 없음",
		describeOcrHealth(st, 60_000, { active: false }) === null,
...
// ---- 한 번도 성공하지 못한 채 시작한 경우 ----
{
	...
	let st = emptyOcrHealth();
	for (let t = 0; t <= 10_000; t += 1000) st = feed(st, LEVEL_GONE, t);
	const notice = describeOcrHealth(st, 10_000, { active: true });
...
// ---- 알림 비교: 초가 바뀌지 않으면 같은 알림 ----
{
	let st = feed(emptyOcrHealth(), OK, 0);
	st = feed(st, BLACK, 1000);
	const a = describeOcrHealth(st, 11_000, { active: true });
	const b = describeOcrHealth(st, 11_400, { active: true });
	const c = describeOcrHealth(st, 12_000, { active: true });
	check("같은 초는 같은 알림", ocrHealthNoticeEquals(a, b), "매 렌더마다 상태가 바뀐 것으로 보이면 안 됩니다");
	check("초가 바뀌면 다른 알림", !ocrHealthNoticeEquals(a, c));
	check("null끼리는 같음", ocrHealthNoticeEquals(null, null));
	check("null과 알림은 다름", !ocrHealthNoticeEquals(a, null));
}
...
// ---- 유예 시간을 직접 넘길 수 있습니다 ----
{
	let st = feed(emptyOcrHealth(), OK, 0);
	st = feed(st, BLACK, 1000);
	check("graceMs 0이면 즉시 경고", describeOcrHealth(st, 1000, { active: true, graceMs: 0 }) != null);
	check("graceMs가 크면 아직 경고 없음", describeOcrHealth(st, 5000, { active: true, graceMs: 60_000 }) === null);
}
